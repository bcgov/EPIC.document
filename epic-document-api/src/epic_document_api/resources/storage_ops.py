...
object_operation_request = ApiHelper.convert_ma_schema_to_restx_model(
    API, ObjectOperationRequestSchema(), "Parameters to perform operations on an S3 object"
)
...
class PresignedURLs(Resource):
    ...
    @ApiHelper.swagger_decorators(
        API, endpoint_description="Get pre-signed urls for uploading files"
    )
    @API.doc(
        params={
            "public-read": {
                "description": "true if you need the url to let you put a file which is available publicly",
                "type": "boolean",
                "required": False,
            },
            "expires": {
...
@cors_preflight("OPTIONS, POST")
@API.route("/objects", methods=["POST", "OPTIONS"])
class ObjectOperations(Resource):
    """Resource for performing operations on objects."""

    @staticmethod
    @ApiHelper.swagger_decorators(API, endpoint_description="Perform operations on an S3 object")
    @API.expect(object_operation_request)
    @API.response(
        code=HTTPStatus.OK,
        description="Successfully performed requested operation",
    )
    @API.response(HTTPStatus.BAD_REQUEST, "Bad Request")
    @cors.crossdomain(origin="*")
    def post():
        """Perform an operation (copy, move, delete, etc.) on an S3 object."""
        request_data = ObjectOperationRequestSchema().load(API.payload)
        result = ObjectStorageService().process_object_operation(request_data)
