...
class ObjectStorageService:
    ...
    def generate_presigned_urls(self, payload: list[dict]):
        """Generate presigned URL based on the given file information."""
        result = []
        documents = []
        for request in payload.get("request_objects"):
            filename = request.get("filename")
            unique_filename = self._generate_unique_filename(filename)
            folder = request.get("folder", "")
            post_url = self.s3_client.generate_presigned_post(
                Bucket=self.s3_bucket,
                Key=os.path.join(folder or "/", unique_filename),
                ExpiresIn=300,
            )
            result.append({filename: post_url})
            documents.append(
                DocumentModel(
                    **{
                        "name": filename,
                        "unique_name": unique_filename,
                        "path": self.get_url(unique_filename, folder),
                        "project_id": request.get("project_id", None),
                    }
                )
            )
        DocumentModel.bulk_insert(documents)
        return result
